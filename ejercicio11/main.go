...
func main() {
	Carlos := new(hombre)
	Carlos.esHombre = true
	HumanosRespirando(Carlos)

	Andrea := new(mujer)
	HumanosRespirando(Andrea)
	...
	PastorAleman := new(perro)
	PastorAleman.carnivoro = true
	PastorAleman.vivo = true
	AnimalesRespirar(PastorAleman)
	totalCarnivoros += AnimalesCarnivoros(PastorAleman)

	fmt.Printf("Total carnivoros %d \n", totalCarnivoros)

	fmt.Printf("Estoy vivo = %t", estoyVivo(Dogo))
	fmt.Printf("Estoy vivo = %t", estoyVivo(Andrea))
